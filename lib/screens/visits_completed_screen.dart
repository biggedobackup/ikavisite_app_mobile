...
class VisitsCompletedScreen extends StatefulWidget {
  const VisitsCompletedScreen({super.key});

  @override
  State<VisitsCompletedScreen> createState() => _VisitsCompletedScreenState();
}

class _VisitsCompletedScreenState extends State<VisitsCompletedScreen> {
  ...
  void _loadVisits() {
    final t = _getToken();
    if (t != null) {
      context.read<VisitProvider>().loadVisitesTerminees(t, onUnauthorized: () async {
  ...
  void _nextPage() async {
    final t = _getToken();
    if (t != null) {
      final err = await context.read<VisitProvider>().nextPageTerminees(t);
      if (err != null && mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<VisitProvider>(
        builder: (_, vp, _) {
          if (vp.error != null && vp.visitsTerminees.isEmpty) {
            return _buildError(vp.error!, _loadVisits);
          }

          if (vp.visitsTerminees.isEmpty && !vp.isRefreshing) {
            return _buildEmpty();
          }

          final hasMore = vp.pageTerminees < vp.totalPagesTerminees;

          return Column(
            children: [
              if (vp.isRefreshing)
                const LinearProgressIndicator(minHeight: 2, backgroundColor: Colors.transparent),
              _buildHeader(vp.countTerminees),
              Expanded(
                child: RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemCount: vp.visitsTerminees.length + (hasMore ? 1 : 0),
                    itemBuilder: (_, i) {
                      if (i == vp.visitsTerminees.length) {
  ...
  Widget _buildHeader(int count) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.list_rounded, size: 16, color: Color(0xFF16A34A)),
  ...
  Widget _buildEmpty() {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Icon(Icons.event_available_rounded, size: 64, color: AppColors.textMuted),
          const SizedBox(height: 16),
          const Text('Aucune visite terminée',
